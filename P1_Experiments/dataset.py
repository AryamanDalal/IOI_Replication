...
class Names:
    def __init__(self):
        ...
        leading space (see IOI._name_tok). This single-token assumption is NOT verified here —
        dataset.py has no tokenizer — so validate it against the model's tokenizer upstream
        (dataset_verification.verify_names does exactly that).
        """
    ...
    def _sample_names(self, seed: int) -> list[str]:
        """
        Obtain a sample of size self.NAMES_PER_BATCH from ALL_NAMES using the given seed. A
        private Random instance keeps the draw reproducible without disturbing global RNG state.

        seed   --> BASE_SEED offset by the batch index, so each batch draws a different sample
        return -> NAMES_PER_BATCH distinct names
        """
    ...
    def sample_name_batches(self) -> list[list[tuple[str, str, str]]]:
        """
        Arrange the names into batches based on the names_per_batch. Each batch is arranged into
        a list of name triplets, one triplet per template.
        return -> NUMBER_OF_BATCHES lists of (NAMES_PER_BATCH // 3) name triples
        """
...
class IOI:
    def __init__(self):
        ...
        self.templates = templates.TEMPLATES             # {small | large | scrambled_small | scrambled_large: list[str]}
        self.ioi_prompts: list[IOIPrompt] = []           # flat record of every IOIPrompt, in build order
    ...
    # -- Section 1: resolve names and roles onto template positions -------------------------
    ...
    def _get_position_to_role(self, template_ordering: str) -> dict[str, str]:
        ...
        The ordering is the only thing that decides whether IO appears before or after S1.

        template_ordering --> "IO_S1_S2" or "S1_IO_S2"
        return -> {"N1": role, "N2": role, "N3": role}
        """
    ...
    def _get_role_to_name(self, name_triplet: tuple[str, str, str]) -> dict[str, dict[str, str]]:
        ...
        Every variant reuses the SAME triplet, so the variants differ only in which name lands
        in which role: corrupt breaks the duplication with the distractor, negative swaps IO
        and S, and scrambled repeats the clean assignment against a scrambled frame.

        name_triplet --> (io_name, s_name, distractor) for this example
        return -> nested dict for each variant with the name assignments for IO, S1, and S2
        """
    ...
    # -- Section 2: fill one template into a Prompt / IOIPrompt -----------------------------
    ...
    def _fill_single_prompt(self, prompt: str, name_triplet: tuple[str, str, str], template_ordering: str, prompt_type: str) -> Prompt:
        ...
        # END fills to "" so the text stops exactly where the model must predict the IO name.
    ...
    def _single_ioi_prompt(self, prompt: str, scrambled_prompt: str, name_triplet: tuple[str, str, str], template_ordering: str, template_size: str) -> IOIPrompt:
        ...
        Only the scrambled variant draws on scrambled_prompt; the other three share one frame,
        which is what makes them comparable to each other.

        prompt           --> the unscrambled template of this size
        scrambled_prompt --> the scrambled template at the same index and size
        return -> IOIPrompt bundling the clean / corrupt / negative / scrambled Prompts
        """
    ...
    # -- Section 3: accumulate prompts into the dataset -------------------------------------
    ...
    def _single_batch_index(self, prompt: str, scrambled_prompt: str, name_triplet: tuple[str, str, str], template_ordering: str, template_size: str) -> None:
        ...
        Appends to self.ioi_prompts and self.prompts. return -> None
        """
    ...
    def _process_batch(self, batch_names: list[tuple[str, str, str]]) -> None:
        ...
        Triplet i is paired with template i, so one batch covers every template exactly once per
        (ordering, size) combination.

        batch_names --> one batch of BATCH_SIZE name triples
        return -> None
        """
    ...
    def create_dataset(self) -> tuple[dict, list[IOIPrompt]]:
        ...
        The leading assert keeps this single-use: calling it twice on one instance would append a
        second copy of every prompt to the accumulators.
        return -> (self.prompts {(ordering, size): {variant: [Prompt]}}, self.ioi_prompts)
        """
    ...
    # TODO: create a shared class and move the prompt parameters to become inputs of the class.
